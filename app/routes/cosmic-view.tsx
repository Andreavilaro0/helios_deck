...
export function loader(_: Route.LoaderArgs) {
  ...
  return { latestSignal, latestSolarWind, latestXRayFlux, latestProtonFlux };
}

export default function CosmicViewRoute({ loaderData }: Route.ComponentProps) {
  const { latestSignal, latestSolarWind, latestXRayFlux, latestProtonFlux } = loaderData;

  if (!latestSignal) {
    return <CosmicEmptyState />;
  }

  return <CosmicScene signal={latestSignal} solarWind={latestSolarWind} xrayFlux={latestXRayFlux} protonFlux={latestProtonFlux} />;
}
...
function CosmicScene({ signal, solarWind, xrayFlux, protonFlux }: CosmicSceneProps) {
  ...
  return <Client signal={signal} solarWind={solarWind} xrayFlux={xrayFlux} protonFlux={protonFlux} />;
}
